...
const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  const renderLists = (item, index) => {
    if (item.type === "item") {
      return <li key={index}>
        <NavLink
          ...
          className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium
               text-bodydark1 duration-300 ease-in-out hover:bg-secondary dark:hover:bg-meta-4 ${pathname.includes(item.path) &&
      ...
      return <SidebarLinkGroup key={index} activeCondition={pathname === item.path || pathname.includes(item.path)}>
        {(handleClick, open) => {
          return (
            <React.Fragment>
              <NavLink
                ...
                className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300
                   ease-in-out hover:bg-secondary dark:hover:bg-meta-4 ${(pathname === item.path ||
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
        ...
        <span className='w-full font-bold text-bodydark1 text-xl'>Question Pool</span>
      ...
      <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
        ...
        <nav className="mt-5 py-4 px-4 lg:mt-9 lg:px-6">
          {/* <!-- Menu Group --> */}
          <div>
            {MenuList?.length > 0 && <ul className="mb-6 flex flex-col gap-1.5">
